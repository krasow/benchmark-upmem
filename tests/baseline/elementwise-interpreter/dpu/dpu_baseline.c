...
int main(void) {                                          
    ...
    for (uint32_t block_loc = tasklet_id << BLOCK_SIZE_LOG2;                
      ...
      // ("complex", "abs(-((a + b) - a))") 
      // I am unrolling this "complex" test to see the effects of the interpreted kernel
      // in libvectordpu and comparing it directly against a compiled kernel.
      ...
      for (uint32_t i = 0; i < block_elems; i++) {                          
        res_block[i] = -res_block[i];                  
      }        

      for (uint32_t i = 0; i < block_elems; i++) {                          
        res_block[i] = res_block[i] - lhs_block[i];                  
      }        

      for (uint32_t i = 0; i < block_elems; i++) {                          
        res_block[i] = abs(res_block[i]);                  
      }        
